...
/**
 * @Title: AccessControlCoprocessor
 * @Description:使用HBase协处理器---基本概念和regionObserver的简单实现
 * @Author: zhaotf
 * @Since:2018年2月12日 下午2:52:49
 * @see {@linkplain https://blogs.apache.org/hbase/entry/coprocessor_introduction}
 * @see {@linkplain https://www.cnblogs.com/ios123/p/6370724.html}
 */
public class AccessControlCoprocessor extends BaseRegionObserver {
	private static final Log LOG = LogFactory.getLog(AccessControlCoprocessor.class);
	...
	public void prePut(final ObserverContext<RegionCoprocessorEnvironment> e, final Put put, final WALEdit edit,
		...
		// 检查A列
		cells = put.get(Bytes.toBytes(FAMAILLY_NAME), Bytes.toBytes(ONLY_PUT_COL));
		if (cells == null || cells.size() == 0) {
		...
		for (Cell cell : cells) {
			try {
				...
				LOG.debug("aValue = " + Bytes.toString(aValue));
		...
		put.addColumn(Bytes.toBytes(FAMAILLY_NAME), Bytes.toBytes(ONLY_READ_COL), aValue);
