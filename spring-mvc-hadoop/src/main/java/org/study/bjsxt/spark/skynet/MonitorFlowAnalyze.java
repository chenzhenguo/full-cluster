...
public class MonitorFlowAnalyze {
	public static void main(String[] args) {
		 ...
		 * 车流量最高的前5个卡扣号
		 */
//		List<String> topNMonitorIds = getTopNMonitorCarFlow(sc, taskId, taskParamsJsonObject, carCount2MonitorRDD);
...
//		carPeng(sqlContext);
	...
	private static void carPeng(SQLContext sqlContext) {
		...
		JavaPairRDD<String, String> car2ActionTime1 = areaRDD1.mapToPair(new PairFunction<Row, String, String>() {
			...
			public Tuple2<String, String> call(Row row) throws Exception {
				...
				return new Tuple2<String, String>(car, Constants.FIELD_MONITOR_ID+"="+row.getAs("monitor_id") + "|" + Constants.FIELD_ACTION_TIME + "=" + row.getAs("action_time"));
			}
		});
		...
		JavaPairRDD<Long, String> minus2InfoRDD = joinRDD.mapToPair(new PairFunction<Tuple2<String,Tuple2<String,String>>, Long, String>() {
			...
			public Tuple2<Long, String> call(Tuple2<String, Tuple2<String, String>> tuple) throws Exception {
				String actionTime1 = StringUtils.getFieldFromConcatString(tuple._2._1,"\\|",Constants.FIELD_ACTION_TIME);
				String actionTime2 = StringUtils.getFieldFromConcatString(tuple._2._2,"\\|",Constants.FIELD_ACTION_TIME);
				Long key = Long.parseLong(DateUtils.minus(actionTime1, actionTime2)+"");
				String monitorId1 = StringUtils.getFieldFromConcatString(tuple._2._1,"\\|",Constants.FIELD_MONITOR_ID);
				String monitorId2 = StringUtils.getFieldFromConcatString(tuple._2._2,"\\|",Constants.FIELD_MONITOR_ID);
				String car = tuple._1;
				String value = Constants.FIELD_CAR + "=" + car + "|" + Constants.FIELD_MONITOR_ID + "=" + monitorId1 + "~" + monitorId2;
				return new Tuple2<Long, String>(key,value);
			}
		});
		
		List<Tuple2<String, String>> takeList = minus2InfoRDD
			.sortByKey()
			.mapToPair(new PairFunction<Tuple2<Long,String>, String,String>() {
				...
				public Tuple2<String, String> call(Tuple2<Long, String> tuple) throws Exception {
					Long minus = tuple._1;
					String monitorIds = StringUtils.getFieldFromConcatString(tuple._2, "\\|", Constants.FIELD_MONITOR_ID);
	...
	private static JavaRDD<Row> getAreaRDDByMonitorIds(SQLContext sqlContext, String startTime, String endTime, List<String> monitorId1) {
		String sql = "SELECT * " + "FROM monitor_flow_action" + " WHERE action_time >" + startTime + " AND action_time < " + endTime + " AND monitor_id in (";
	 ...
	 */
	private static void getMonitorDetails(JavaSparkContext sc, final long taskId, List<String> top5MonitorIds, JavaPairRDD<String, Iterable<Row>> monitor2RowsRDD) {
	...
	@SuppressWarnings("resource")
	private static void getTopNDetails(final long taskId, List<String> topNMonitorIds, JavaPairRDD<String, Iterable<Row>> monitor2RowsRDD) {
		/**
			...
			public void call(Iterator<Tuple2<String, Iterable<Row>>> iterator) throws Exception {
				...
				while (iterator.hasNext()) {
					  ...
					  while(iterator2.hasNext()){
						   ...
						   monitorDetailInfos.add(new TopNMonitorDetailInfo(taskId, row.getAs("date")+"", row.getAs("monitor_id")+"", row.getAs("camera_id")+"", row.getAs("car")+"", row.getAs("action_time")+"", row.getAs("speed")+"", row.getAs("road_id")+""));
					  }
				}
				IMonitorDAO monitorDAO = DAOFactory.getMonitorDAO();
	 ...
	 */
	private static List<String> getTopNMonitorCarFlow(JavaSparkContext sc, long taskId, JSONObject taskParamsJsonObject, JavaPairRDD<Integer, String> carCount2MonitorId) {
		/**
			...
			public Iterator<Tuple2<Integer, String>> call(Iterator<Tuple2<String, Tuple2<String, Optional<String>>>> iterator) throws Exception {
				...
				while (iterator.hasNext()) {
					...
					if (factCameraInfosOptional.isPresent()) {
						...
						monitorAndCameraStateAccumulator.add(Constants.FIELD_ABNORMAL_MONITOR_COUNT + "=1|" + Constants.FIELD_ABNORMAL_CAMERA_COUNT + "=" + abnoramlCameraCount + "|" + Constants.FIELD_ABNORMAL_MONITOR_CAMERA_INFOS + "=" + monitorId + ":" + standardCameraIds);
						continue;
					}
					/**
					 * 从实际数据拼接的字符串中获取摄像头数
