 ...
 * @Description:
 * @Author: zhaotf
 * @Since:2018年3月9日 上午7:25:43
 * @see {@linkplain https://www.cnblogs.com/itboys/p/6674132.html}
 */
public class JavaWordCount {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * 在创建上下文的过程中，程序会向集群申请资源及构建相应的运行环境。 设置spark应用程序名称 创建的 sarpkContext
		 * 唯一需要的参数就是 sparkConf，它是一组 K-V 属性对。
		 */
		 ...
		 * RDD的初始创建都是由SparkContext来负责的，将内存中的集合或者外部文件系统作为输入源。 RDD：弹性分布式数据集，即一个
		 * RDD 代表一个被分区的只读数据集。一个 RDD 的生成只有两种途径， 一是来自于内存集合和外部存储系统，另一种是通过转换操作来自于其他
		 * RDD，比如 Map、Filter、Join，等等。
		 * textFile()方法可将本地文件或HDFS文件转换成RDD，读取本地文件需要各节点上都存在，或者通过网络共享该文件 读取一行
		 */
		 ...
		 * 表示输入类型为T,生成的key-value对中的key类型为k,value类型为v,对本例,T=String, K=String,
		 * V=Integer(计数) 需要重写call方法实现转换
		 */
		...
		/**
		 * 调用reduceByKey方法,按key值进行reduce reduceByKey方法，类似于MR的reduce
		 * 要求被操作的数据（即下面实例中的ones）是KV键值对形式，该方法会按照key相同的进行聚合，在两两运算 若ones有<"one",
		 * 1>, <"one", 1>,会根据"one"将相同的pair单词个数进行统计,输入为Integer,输出也为Integer
		 * 输出<"one", 2>
		 */
